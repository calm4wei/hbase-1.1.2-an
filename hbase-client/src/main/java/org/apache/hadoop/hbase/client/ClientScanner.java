...
public class ClientScanner extends AbstractClientScanner {
    ...
    public ClientScanner(final Configuration conf, final Scan scan, final TableName tableName,
            ClusterConnection connection, RpcRetryingCallerFactory rpcFactory, RpcControllerFactory controllerFactory,
        ...
        this.scannerTimeout = HBaseConfiguration.getInt(conf, HConstants.HBASE_CLIENT_SCANNER_TIMEOUT_PERIOD,
        ...
        if (this.scan.getCaching() > 0) {
            ...
            this.caching = conf
                    .getInt(HConstants.HBASE_CLIENT_SCANNER_CACHING, HConstants.DEFAULT_HBASE_CLIENT_SCANNER_CACHING);
        }
    ...
    private boolean possiblyNextScanner(int nbRows, final boolean done) throws IOException {
        ...
        if (callable != null && callable.switchedToADifferentReplica())
            return true;
    ...
    protected boolean nextScanner(int nbRows, final boolean done) throws IOException {
        ...
        if (this.currentRegion != null) {
            ...
            if (endKey == null || Bytes.equals(endKey, HConstants.EMPTY_BYTE_ARRAY) || checkScanStopRow(endKey)
                    || done) {
                close();
                if (LOG.isTraceEnabled()) {
    ...
    Result[] call(ScannerCallableWithReplicas callable, RpcRetryingCaller<Result[]> caller, int scannerTimeout)
    ...
    protected ScannerCallableWithReplicas getScannerCallable(byte[] localStartKey, int nbRows) {
        ...
        s.setCaching(nbRows);
        ScannerCallableWithReplicas sr = new ScannerCallableWithReplicas(tableName, getConnection(), s, pool,
                primaryOperationTimeout, scan, retries, scannerTimeout, caching, conf, caller);
        return sr;
    }
    ...
    protected void loadCache() throws IOException {
        ...
        do {
                ...
                if (e instanceof UnknownScannerException) {
                    ...
                    if (timeout < System.currentTimeMillis()) {
                        ...
                        ScannerTimeoutException ex = new ScannerTimeoutException(
                                elapsed + "ms passed since the last invocation, " + "timeout is currently set to "
                                        + scannerTimeout);
                        ex.initCause(e);
                        throw ex;
                    }
                    ...
                    if ((cause != null && cause instanceof NotServingRegionException) || (cause != null
            ...
            // row.
        } while (doneWithRegion(remainingResultSize, countdown, serverHasMoreResults) && (!partialResults.isEmpty()
     ...
     * @throws IOException
     */
    protected List<Result> getResultsToAddToCache(Result[] resultsFromServer, boolean heartbeatMessage)
            throws IOException {
    ...
    private void addToPartialResults(final Result result) throws IOException {
        ...
        if (partialResultsRow != null && !Bytes.equals(row, partialResultsRow)) {
            throw new IOException("Partial result row does not match. All partial results must come "
                    + "from the same row. partialResultsRow: " + Bytes.toString(partialResultsRow) + "row: " + Bytes
                    .toString(row));
        }
